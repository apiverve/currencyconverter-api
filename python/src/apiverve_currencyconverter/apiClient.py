...
class CurrencyconverterAPIClientError(Exception):
...
class ValidationError(CurrencyconverterAPIClientError):
...
class CurrencyconverterAPIClient:
    ...
    VALIDATION_RULES = {"value": {"type": "number", "required": True, "min": 0}, "from": {"type": "string", "required": True}, "to": {"type": "string", "required": True}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the CurrencyconverterAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises CurrencyconverterAPIClientError: If API key is invalid
        """
    ...
    def _validate_api_key(self, api_key: str) -> None:
        ...
        if not api_key or not api_key.strip():
            raise CurrencyconverterAPIClientError(
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[CurrencyconverterAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns CurrencyConverterData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises CurrencyconverterAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise CurrencyconverterAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise CurrencyconverterAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise CurrencyconverterAPIClientError(f"Request failed: {str(e)}")
