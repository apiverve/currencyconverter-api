...
class CurrencyconverterValidationException implements Exception {
  final List<String> errors;

  CurrencyconverterValidationException(this.errors);

  @override
  String toString() => 'CurrencyconverterValidationException: ${errors.join("; ")}';
}
...
///
/// For more information, visit: https://currencyconverter.apiverve.com?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [value] (required) - The amount to convert [min: 0]
/// * [from] (required) - The ISO 4217 currency code to convert from
/// * [to] (required) - The ISO 4217 currency code to convert to
class CurrencyconverterClient {
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'value': ValidationRule(type: 'number', required: true, min: 0),
    'from': ValidationRule(type: 'string', required: true),
    'to': ValidationRule(type: 'string', required: true),
  };

  CurrencyconverterClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/currencyconverter',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CurrencyconverterValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CurrencyconverterValidationException(errors);
    }
  }

  /// Execute a request to the Currency Converter API.
  ///
  /// Parameters are validated before sending the request.
  Future<CurrencyconverterResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CurrencyconverterException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CurrencyconverterResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CurrencyconverterException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CurrencyconverterException('Resource not found');
      } else {
        throw CurrencyconverterException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CurrencyconverterException) rethrow;
      throw CurrencyconverterException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Currency Converter API client.
class CurrencyconverterException implements Exception {
  final String message;

  CurrencyconverterException(this.message);

  @override
  String toString() => 'CurrencyconverterException: $message';
}
